...
- **The optional tier is a pair per component.** Each elected component (dex, flux-web, patchy) is a cloud-neutral
  ...
  cloud's secret-sync dialect and its Gateway edge-admission dialect (on google the GKE Gateway `HealthCheckPolicy` set
  plus a GFE-range `NetworkPolicy`; on aws a `CiliumNetworkPolicy` admitting the Cilium Gateway's reserved `ingress`
  identity, which no Kubernetes `NetworkPolicy` selector can name). The core's Kustomization depends on its companion,
  so namespaces and credentials exist before the release reconciles.
...
| flux                 | common   | flux managing flux: the operator + FluxInstance HelmReleases adopt terraform's bootstrap releases and follow the newest mirrored charts; `cluster.type` rides `OCI_PROVIDER` (the value sets coincide), and `sync.path` — the fixed point that keeps a running cluster on its own tree — is re-asserted by each tree's `flux.yaml` entry via an inline `postBuild.substitute` (`FLUX_SYNC_PATH`) |
